...
@PluginDescriptor(name = "One Tap Agility", description = "Reclined gaming", enabledByDefault = false)
class OneTapAgilityPlugin : Plugin() {

    var gameEventManager = GameEventManager
    var itemManager = ItemManager
    private val config = configuration<OneTapAgilityConfig>()
    ...
    override fun onClientTick(it: ClientTick) {
        ...
        val text: String = if (course?.getCurrentObstacleArea(client.localPlayer) == null) {
            if (config.consumeMisclicks.get()!!) {
                "One Click Agility"
            } else {
                return
            }
    ...
    private fun handleClick(it: MenuOptionClicked) {
        ...
        if (config.seersTele.get()!! && config.courseSelection.get()!! == AgilityCourse.SEERS_VILLAGE) {   //spellbook varbit, worldpoint of dropdown tile, teleportation animation ID
        ...
        if (config.courseSelection.get()!! == AgilityCourse.AGILITY_PYRAMID) {
        ...
        if (config.pickUpMarks.get()!! && !marks.isEmpty()) {
    ...
    override fun onCheckClick(it: CheckClick) {
        if (config.pickUpMarks.get()!! && marks.isNotEmpty()) {
