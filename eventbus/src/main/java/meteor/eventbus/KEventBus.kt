 ...
 * @param isSticky whether this event is sticky. Sticky event will be stored in the [KEventBus]
 * until it get removed manually by calling [KEventBus.removeStickyEvent], [KEventBus.removeAllStickyEvents] or [KEventBus.clear].
...
data class Event<T : Any>(
    ...
    fun isPostedSticky(kEvent: KEventBus = KEVENT) : Boolean = kEvent.containsStickyEvent(this as Event<Any>)
}
 ...
 * @param tag optional tag that is useful for unsubscription, see [KEventBus.removeSubscribersByTag].
 */
...
class KEventBus(
    ...
    /**
     * Clear all states of the [KEventBus] object. All subscribers, sticky events, event blocking, etc will be removed.
     */
    fun clear() {
