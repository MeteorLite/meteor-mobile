...
import meteor.Main
import meteor.Main.platformPlugins
...
import meteor.plugins.nmzafk.NightmareZoneAFKPlugin
    ...
    init {
        ...
        if (pluginsEnabled) {
            ...
            for (plugin in platformPlugins)
                initPlatformPlugin(plugin)
        }
        plugins.sortBy { it.getName() }
        Main.logger.warn("Loaded ${plugins.size} plugins")
    }

    fun initPlatformPlugin(plugin: Class<out Plugin>) {
        val plugin = plugin.getDeclaredConstructor().newInstance()
